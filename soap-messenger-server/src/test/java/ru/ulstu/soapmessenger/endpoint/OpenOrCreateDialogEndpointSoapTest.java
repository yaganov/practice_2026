...
@WebServiceServerTest(OpenOrCreateDialogEndpoint.class)
@Import(OpenOrCreateDialogEndpointSoapTest.TestConfig.class)
class OpenOrCreateDialogEndpointSoapTest {
	...
	@Test
	void openOrCreateDialog_success() throws Exception {
		...
		when(dialogService.openOrCreateDialog(eq(currentUserId), eq(otherUserId))).thenReturn(dialogSummary);

		withBearerToken(createToken(currentUserId));
		client.sendRequest(withSoapEnvelope(openOrCreateDialogEnvelope(otherUserId)))
				.andExpect(noFault())
				.andExpect(xpath("//tns:OpenOrCreateDialogResponse/tns:dialog/tns:dialogId", NAMESPACES)
						.evaluatesTo(dialogId.toString()))
				.andExpect(xpath("//tns:OpenOrCreateDialogResponse/tns:dialog/tns:interlocutor/tns:userId",
						NAMESPACES).evaluatesTo(otherUserId.toString()))
				.andExpect(xpath("//tns:OpenOrCreateDialogResponse/tns:dialog/tns:interlocutor/tns:username",
						NAMESPACES).evaluatesTo("bob"));
	}

	@Test
	void openOrCreateDialog_unauthorizedFault() throws Exception {
		client.sendRequest(withSoapEnvelope(openOrCreateDialogEnvelope(UUID.randomUUID())))
				.andExpect(clientOrSenderFault("Требуется действующий токен авторизации"))
				.andExpect(xpath("//soapenv:Fault/detail/tns:OpenOrCreateDialogFault/tns:code", NAMESPACES)
						.evaluatesTo("UNAUTHORIZED"));
	}

	@Test
	void openOrCreateDialog_userNotFoundFault() throws Exception {
		UUID currentUserId = UUID.randomUUID();
		UUID otherUserId = UUID.randomUUID();

		when(dialogService.openOrCreateDialog(eq(currentUserId), eq(otherUserId)))
	...
	private Source openOrCreateDialogEnvelope(UUID otherUserId) {
				...
				<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:tns="urn:soap-messenger:v1">
				  <soapenv:Body>
				    <tns:OpenOrCreateDialogRequest>
				      <tns:otherUserId>%s</tns:otherUserId>
				    </tns:OpenOrCreateDialogRequest>
				  </soapenv:Body>
				</soapenv:Envelope>
	...
	@Configuration
	@Import({OpenOrCreateDialogEndpoint.class, SecurityConfig.class})
	static class TestConfig {
		...
		@Bean
		PayloadRootSmartSoapEndpointInterceptor openOrCreateDialogJwtAuthenticationInterceptor(
			...
			return new PayloadRootSmartSoapEndpointInterceptor(jwtAuthenticationInterceptor,
					"urn:soap-messenger:v1", "OpenOrCreateDialogRequest");
		}

		@Bean
		SoapFaultExceptionResolver openOrCreateDialogFaultExceptionResolver(
